<template>
  <div class="page">
    <div class="page-header"><h2>行动项</h2></div>
    <div class="list">
      <div class="list-item" v-for="item in items" :key="item.id">
        <div class="list-item-main">
          <span class="item-title">{{ item.title }}</span>
          <span class="item-meta">{{ item.priority }} · {{ item.assignee_name || '未分配' }}</span>
        </div>
        <span class="badge" :class="item.status">{{ item.status }}</span>
      </div>
      <p v-if="!items.length" class="empty">暂无行动项</p>
    </div>
  </div>
</template>
...
.badge.overdue { background: #fff1f0; color: #f5222d; }
